Feature: End-to-End process of group registering for a conference by a registrant when required seats are not available
...
#   3. the requested number of seats are not available
...
#note this should be the case always
...
#Note -  Question when a seat type is sold out-  means there is no seats in the seat type, but still the user can request for one and he can be offered waitlisted , right?
...
Scenario: Make a registration order selection
	...
	And only 1 seat available in General admission seat type
	When I give the below details
	| seat type                        | number of seats |
	| General admission                | 2				 |
	| Additional cocktail party		   | 2				 |
	
	Then I should see a message that only 1 seat is available
	And I should be provided a waitlist offer

Scenario: Accept waitlist offer and checkout
Given the following seats are available
 
    | seat type                        | number of seats |
	| General admission                | 1				 |
	| Additional cocktail party		   | 2				 |

And I accepted waitlist offer for 1 General admission seat
Then I should see the total of $299
